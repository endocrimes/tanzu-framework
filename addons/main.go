...
func main() {
	...
	antreaConfigReconciler := &antrea.AntreaConfigReconciler{
		Client: mgr.GetClient(),
		Log:    ctrl.Log.WithName("controllers").WithName("AntreaConfig"),
		Scheme: mgr.GetScheme(),
	}

	if err = antreaConfigReconciler.SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "AntreaConfig")
		os.Exit(1)
	}
